...
const AdminHomePage = () => {
  const adminItems = [
    { icon: "👨‍⚕️", title: "Add Doctor", path: "/DoctorEntry", variant: "primary" },
    ...
    { icon: "🛏️", title: "Room Allocation", path: "/RoomAllocation", variant: "secondary" },
    { icon: "🏢", title: "Add Rooms", path: "/createRoom", variant: "accent" },
    { icon: "🔔", title: "Notifications", path: "/notification-add", variant: "primary" },
    { icon: "🤖", title: "HospiAI", path: "/prompt", variant: "secondary" },
  ];
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {adminItems.map((item, index) => (
            <AdminCard
              ...
              icon={item.icon}
              title={item.title}
              path={item.path} // navigation works now
